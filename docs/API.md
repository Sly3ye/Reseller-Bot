...
| `/api/depreciation?category=...` | GET | Curva di deprezzamento per variante: mediana per età del modello, perdita a 12 mesi, costo di magazzino, valore residuo fra generazioni |
| `/api/time-to-sale?category=...` | GET | Fatti grezzi dei venduti (modello, colore, taglia, giorni, prezzo) per il pivot dei giorni di vendita |
...
| `test_intelligence.py` / `test_variants.py` / `test_valuation.py` / `test_health.py` / `test_depreciation.py` / `test_db_shim.py` | Test delle funzioni pure (NLP, scoring, varianti, valutazione, salute, deprezzamento, layer DB) |
