...
Esegui dalla root:
  python scripts/run_backfill.py [max_pages_per_target] [category] [filtro_query]
  es. python scripts/run_backfill.py 10 automobile
      python scripts/run_backfill.py 8 smartphone "iphone 17"   # solo i target nuovi
"""
...
async def backfill_target(
    ...
    # La sessione HTTP la apre e chiude _get_with_retry, una per tentativo:
    # è così che ogni richiesta esce da un nodo residenziale diverso.
    start = 0
...
async def main() -> None:
    ...
    # Filtro sulla query: serve quando aggiungi target nuovi e vuoi riempire
    # SOLO quelli, senza ri-aspirare tutta la flotta (e il budget del proxy).
    like = sys.argv[3].lower() if len(sys.argv) > 3 else None

    scraper = SubitoScraper()
    targets = await asyncio.to_thread(get_active_targets, category)
    if like:
        targets = [t for t in targets if like in t["query"].lower()]
    scope = f"categoria '{category}'" if category else "tutte le categorie"
    if like:
        scope += f", query che contengono '{like}'"
